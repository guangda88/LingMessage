...
class CapabilityRegistry:
    ...
    def migrate_from_lingflow_plus(self) -> int:
        """Migrate static MCP_SERVERS from lingflow_plus.mcp_registry.

        Imports all server configs from the hardcoded dictionary in
        lingflow_plus, converting them to ServerCapability entries.
        Existing entries in this registry are preserved (no overwrite).
        ...
        try:
            ...
            logger.warning("lingflow_plus.mcp_registry not available, skipping migration")
            return 0
        ...
        for key, config in MCP_SERVERS.items():
            ...
            cap = ServerCapability(
                ...
                tools=tuple(config.tools) if config.tools else (),
                transport=config.transport.value if hasattr(config.transport, "value") else str(config.transport),
                command=command,
                args=tuple(args),
                url=config.url or "",
                working_dir=working_dir,
                registered_at=_now_epoch_iso(),
                last_heartbeat=_now_epoch_iso(),
            )
            self._servers[key] = cap
            migrated += 1
        ...
        if migrated:
            self._save()
            logger.info("Migrated %d MCP servers from lingflow_plus", migrated)
        return migrated
