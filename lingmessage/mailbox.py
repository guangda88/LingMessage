...
class Mailbox:
    ...
    _SAFE_THREAD_ID_RE = re.compile(r"^(disc_)?[0-9a-zA-Z_]{4,64}$")
    ...
    @staticmethod
    def _is_safe_thread_id(value: str) -> bool:
        """Check whether a thread ID is safe (hex UUID or disc_ prefixed)."""
        return bool(Mailbox._SAFE_THREAD_ID_RE.fullmatch(value))
    ...
    def _validate_thread_id(value: str) -> None:
        """Validate that a thread ID is safe (hex UUID or disc_ prefixed)."""
        if not Mailbox._SAFE_THREAD_ID_RE.fullmatch(value):
            raise ValueError(f"Invalid thread_id: unsafe characters, got {value!r}")
    ...
    def post(self, message: Message, signature: str = "") -> Message:
        ...
        if message.source_type == SourceType.VERIFIED:
            ...
            message = replace(
                message,
                source_trace=f"signature:{computed}",
            )
